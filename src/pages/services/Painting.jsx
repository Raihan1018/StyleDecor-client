...
// Data structure for the painting plans
const plansData = [
  {
    name: "Touch-Up Package",
    priceMonthly: 5000,
    features: [
      "Small area touch-up (up to 100 sq. ft.)",
      "Minor wall preparation (patching)",
      "Single color application",
      "Trim and door paint (1 unit)",
    ],
  },
  {
    name: "Room Revamp",
    priceMonthly: 12000,
    priceDiscountedMonthly: 11000, // Monthly Discount
    features: [
      "Single room painting (standard size)",
      "Detailed wall preparation (sanding, multiple patching)",
      "Color consultation included",
      "Two-coat coverage for superior finish",
      "Complete cleanup and furniture replacement",
    ],
  },
  {
    name: "Full Interior Makeover",
    priceMonthly: 25000,
    priceDiscountedMonthly: 23500, // Monthly Discount
    features: [
      "Multiple rooms or full apartment/house interior (up to 1500 sq. ft.)",
      "Premium paint selection guidance",
      "Trim, doors, and window frame detailing",
      "Wallpaper removal (basic areas)",
      "Project management and guaranteed completion date",
    ],
  },
];

// Helper component for the checkmark SVG
const CheckIcon = ({ className = "w-6 h-6 text-red-600" }) => (
  <svg
...
const PricingCard = ({ plan, isMonthly, yearlyDiscount, color }) => {
  ...
  return (
    <div className={cardClasses}>
      {/* Name Tag */}
      <span className={`absolute top-0 px-6 pt-1 pb-2 font-medium rounded-b-lg ${colorClasses.split(' ').slice(0, 2).join(' ')}`}>
        {plan.name}
      </span>
      ...
      <p className="flex items-center justify-center my-6 space-x-2 font-bold mt-16">
        ...
        <span className={`pb-2 text-4xl ${colorClasses.split(' ').slice(2).join(' ')}`}>
...
const Painting = () => {
  ...
  const color = "red"; // Theme color for Painting

  // Button classes (Simplified for White Theme)
  const activeBtnClasses =
    `px-4 py-1 font-semibold border rounded-l-lg bg-${color}-600 border-${color}-600 text-white transition-colors`;
  const inactiveBtnClasses =
    `px-4 py-1 border rounded-r-lg border-${color}-600 text-gray-800 transition-colors`;
        ...
        <div className="container mx-auto p-4 sm:p-10">
          <div className="mb-16 space-y-4 text-center">
            <h1 className="text-4xl font-semibold leading-tight">
              Painting & Wall Service Pricing
            </h1>
            <p className="px-4 sm:px-8 lg:px-24">
              Professional painting services for a flawless, vibrant home interior or exterior.
            </p>
            {/* Toggler Button Group */}
            <div className={`inline-flex border-2 rounded-lg border-${color}-600`}>
              <button
            ...
            {!isMonthly && (
              <p className={`text-sm text-${color}-600 font-bold mt-2`}>
                * You save an extra {yearlyDiscountRate * 100}% with the Annual Plan!
              </p>
            )}
          </div>
