...
// Helper to style category buttons
...
const AllServices = () => {
  ...
  // Calculate final price per hour in BDT
  const calculateFinalPrice = (price, discount) => {
    const basePrice = Number(price) || 0;
    const discountRate = (Number(discount) || 0) / 100;
            ...
            <div className="flex flex-wrap gap-2 justify-center lg:justify-start p-1 bg-gray-50 rounded-lg border border-gray-200 overflow-x-auto">
              {categories.map((cat) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  {cat === "All" && <FaTags className="inline-block mr-1" />}
                  {cat}
                </motion.button>
              ))}
            </div>
                  ...
                  <div className="flex items-center gap-3">
                    {plan.discount > 0 && (
                      <span className="text-sm text-gray-400 line-through">
                        BDT {Number(plan.price).toFixed(2)}
                      </span>
                    )}
                    <span className="text-4xl font-extrabold text-blue-600">
                      BDT {calculateFinalPrice(plan.price, plan.discount)}
                    </span>
